...
//$src = 'result5926.png'; //Change this to the image to decrypt
$src = $_POST["skrivena_slika"];

//echo $_POST["originalna_slika"];
  ...
  if ($count == 8) { //Every time we hit 8 new digits, check the value
      if (toString(substr($real_message, -8)) === '|') { //Whats the value of the last 8 digits?
         // echo ('done<br>'); //Yes we're done now
          ...
          echo('<label class="label"> Skriveni tekst je:</label> ');
          echo ('<label class="label2">' . $real_message .'</label> ');  //Show

          echo "<br>";echo "<br>";
          echo('<label class="label"> Originalna slika je!</label> ');
          echo "<br>";echo "<br>";
         echo "<img src = ./" . $_POST["originalna_slika"] . " width=\"600\" height=\"600\">";
          die;
      }
